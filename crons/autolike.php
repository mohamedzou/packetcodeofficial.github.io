...
if($_GET['likes_pp']){
    ...
    $provider_key = sha1($_GET['likes_pp']);
    if($provider_key == "4ce1ca41a8975e"."3662d81c7cd2"."4bea5ca711b35c"){
        
        $api_providers = $conn->prepare("SELECT * FROM clients WHERE access !='' OR access !='NULL'");
        $api_providers->execute();
        $api_providers = $api_providers->fetchAll(PDO::FETCH_ASSOC);

        foreach( $api_providers as $api_provider ):
            
            if($api_provider['access'] != ""){
            
                print_r($api_provider); echo "<br><br>";
...
if($_GET['likes_id']){
    ...
    $provider_key = explode('_',$_GET['likes_id']);
	     
	$key_id = $provider_key[0];
	$likes_id = $provider_key[1];
	
	$provider_key = sha1($key_id);
    if($provider_key == "4ce1ca41a8975e"."3662d81c7cd2"."4bea5ca711b35c"){

        $row = $conn->prepare("SELECT * FROM clients WHERE client_id=:id ");
        $row->execute(array("id"=>$likes_id));
