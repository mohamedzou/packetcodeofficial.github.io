...
// $orderstatus= $smmapi->action(array('key' =>"73811a037f011ff63776b6900273e6f1",'action' =>'status','order'=>"83148"),"https://smmfocus.com/api/v2");
  ...
  foreach( $orders as $order ):
    ...
    if( empty( $finish ) || !$finish ):
      ...
      elseif( $statu == 'complete' || $statu == 'completed' ):
        ...
        $update = $update->execute(array("start"=>$start,"finish"=>$finish,"status"=>"completed","detail"=>json_encode($orderstatus),"id"=>$orderid,"order_profit"=>$order["api_currencycharge"]*$api_charge,"api_charge"=>$api_charge,"remains"=>$remains));
          if( $update && $report ):
            $conn->commit();
          else:
            $conn->rollBack();
      ...
      elseif( $statu == 'pending' ):
        ...
        $conn->beginTransaction();
        $update = $conn->prepare("UPDATE orders SET order_start=:start, order_finish=:finish, order_remains=:remains, order_status=:status, order_detail=:detail, api_charge=:api_charge, order_profit=:order_profit WHERE order_id=:id ");
        $update = $update->execute(array("start"=>$start,"finish"=>$finish,"remains"=>$remains,"status"=>"pending","detail"=>json_encode($orderstatus),"id"=>$orderid,"order_profit"=>$order["api_currencycharge"]*$api_charge,"api_charge"=>$api_charge));
          if( $update ):
            $conn->commit();
          else:
            $conn->rollBack();
      ...
      elseif( $statu == 'inprogress' ):
        ...
        $conn->beginTransaction();
        $update = $conn->prepare("UPDATE orders SET order_start=:start, order_finish=:finish, order_remains=:remains, order_status=:status, order_detail=:detail, api_charge=:api_charge, order_profit=:order_profit WHERE order_id=:id ");
        $update = $update->execute(array("start"=>$start,"finish"=>$finish,"remains"=>$remains,"status"=>"inprogress","detail"=>json_encode($orderstatus),"id"=>$orderid,"order_profit"=>$order["api_currencycharge"]*$api_charge,"api_charge"=>$api_charge));
          if( $update ):
            $conn->commit();
          else:
            $conn->rollBack();
      ...
      elseif( $statu == 'processing' ):
        ...
        $conn->beginTransaction();
        $update = $conn->prepare("UPDATE orders SET order_start=:start, order_finish=:finish, order_remains=:remains, order_status=:status, order_detail=:detail, api_charge=:api_charge, order_profit=:order_profit WHERE order_id=:id ");
        $update = $update->execute(array("start"=>$start,"finish"=>$finish,"remains"=>$remains,"status"=>"processing","detail"=>json_encode($orderstatus),"id"=>$orderid,"order_profit"=>$order["api_currencycharge"]*$api_charge,"api_charge"=>$api_charge));
          if( $update ):
            $conn->commit();
          else:
            $conn->rollBack();
