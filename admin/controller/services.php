<?php
  ...
  elseif( $action == "new-service" ):
      if( $_POST ):
        ...
        elseif( !is_numeric($price) ):
          $error    = 1;
          $errorText= "The price should consist of numbers !";
          $icon     = "error";
        else:
            ...
            $insert = $insert-> execute(array("secret"=>$secret,"multiName"=>$multiName,"instagram_second"=>$instagram_second,"dripfeed"=>$dripfeed,"start_count"=>$start_count,"instagram_private"=>$instagram_private,"api"=>$provider,"api_service"=>$service,"detail"=>$detail,"category"=>$cat,"line"=>$row["service_line"]+1,"type"=>2,"package"=>$package,"name"=>$name,"description" => $description,"price"=>$price,"min"=>$min,"max"=>$max,"want_username"=>$want_username,"speed"=>$speed ));
            if( $insert ):
              $conn->commit();
      ...
      if( $_POST ):
        ...
        if( empty($name) ):
          ...
          $row = $conn->query("SELECT * FROM categories ORDER BY category_line DESC LIMIT 1 ")->fetch(PDO::FETCH_ASSOC);
           ...
           $nweIcon = $icon!=""?$icon:" ";
            $insert = $conn->prepare("INSERT INTO categories SET category_name=:name, category_line=:category_line, category_icon=:icon, category_secret=:secret ");
            $insert = $insert-> execute(array("name"=>$name,"secret"=>$secret,"icon"=>$nweIcon,"category_line"=>$category_line ));
            if( $insert ):
      ...
      if( $_POST ):
        ...
        if( empty($name) ):
          ...
          $icon     = "error";
            ...
            $update = $conn->prepare("UPDATE categories SET category_name=:name, category_icon=:icon, category_secret=:secret,category_line=:category_line WHERE category_id=:id  ");
        $icon = $icon!=""?$icon:" ";
            $update = $update->execute(array("name"=>$name,"secret"=>$secret,"icon"=>$icon,"id"=>$category_id,"category_line"=>$category_line ));
  ...
  elseif( $action == "category-delete" ):
    $category_id  = route(3);
    $update = $conn->prepare("DELETE FROM categories WHERE category_id=:id ");
              ...
              ///searched by omi
               ...
               if (!countRow(["table" => "categories", "where" => ["category_name" => $apiService->category]])){
                ...
                if ($package == 11):
                            ...
                            $fpcarray =$conn->prepare("INSERT INTO services (service_api, api_service, category_id, service_line, service_type, service_package, service_name, service_price, service_min, service_max) VALUES ('$provider_id','$service','$getcatidlast',1,2,'$package','$servicename','$price','$apiService->min','$apiService->max');");
                            $fpcarray=$fpcarray->execute();
                        else:
                            $getcatidlast = $getcatid['category_id'];
                            $details = json_encode($detail);
                            $fpcarray = $conn->prepare("INSERT INTO services (service_api, api_service, api_detail, category_id, service_line, service_type, service_package, service_name, service_price, service_min, service_max) VALUES ('$provider_id','$service','$details','$getcatidlast',1,2,'$package','$servicename','$price','$apiService->min','$apiService->max');");
                            $fpcarray = $fpcarray->execute();
              /*
                  if( $package == 11 ):
